...
enum PlasticityMessage {
    ...
    Batch(Vec<Event>),
    Sync(mpsc::Sender<()>), // <-- NEW: The synchronization ping
}
...
impl Plasticity {
            ...
            while let Ok(message) = rx.recv() {
                match message {
                    ...
                    PlasticityMessage::Sync(reply_tx) => {
                        // The background thread has processed all prior events in the queue!
                        // Send the ping back to the conscious mind so it can wake up.
                        let _ = reply_tx.send(());
                    }
                }
            }
    ...
    /// Explicitly tells the conscious mind to wait until the subconscious thread
    /// has finished processing every event currently in its queue.
    pub fn sync(&self) {
        let (tx, rx) = mpsc::channel();
        let _ = self.sender.send(PlasticityMessage::Sync(tx));
        let _ = rx.recv(); // Block until the background thread responds
    }
    ...
    pub fn get_op_distribution(&self, context_event: Event, allowed_ops: &[Op]) -> Vec<(Op, f32)> {
